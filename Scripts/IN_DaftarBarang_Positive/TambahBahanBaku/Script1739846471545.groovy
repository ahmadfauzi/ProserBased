...
WebUI.sendKeys(findTestObject('Object Repository/Page_TataPabrik/DaftarBarang/input_Rp_price'), Keys.chord(Keys.CONTROL, 'a'))
...
if (netto == '' && WebUI.verifyElementPresent(findTestObject('DaftarBarang/Page_TataPabrik/span_Wajib diisi_netto'), 2)) {
	KeywordUtil.logInfo("'Netto is empty and the warning text is displayed")
	isErrorPresent = true
}
...
if(getOnlyNumberNetto == '' && WebUI.verifyElementPresent(findTestObject('DaftarBarang/Page_TataPabrik/span_Wajib diisi_netto'), 2)) {
	KeywordUtil.logInfo("Netto is not number and the warning text is displayed")
	isErrorPresent = true
}

String inputHarga = harga;
KeywordUtil.logInfo("Harga input = "  + inputHarga)
String getOnlyNumberHarga = inputNetto.replaceAll('[^0-9]', '')
KeywordUtil.logInfo("get only number =  "  + getOnlyNumberHarga)
if(getOnlyNumberHarga == '') {
	KeywordUtil.logInfo("Harga is not number")
	isErrorPresent = true
}
...
if (!isErrorPresent){
	if (WebUI.waitForElementPresent(findTestObject('DaftarBarang/Page_TataPabrik/div_AlertDialog_duplicate'), 2)) {
		...
		String hargaTabelText = WebUI.getText(findTestObject('DaftarBarang/Page_TataPabrik/div_list_satuanberat'))
		
		String partsTabel = hargaTabelText.split(',00')[0]
		
		String numericTabelText = partsTabel.replaceAll('[^0-9]', '')
		
		String hargaInputText = harga
		
		String partsHarga = hargaInputText.split(',00')[0]
		
		String numericTabelHarga = partsHarga.replaceAll('[^0-9]', '')
		
		WebUI.verifyMatch(numericTabelText, numericTabelHarga, false)
	}
}